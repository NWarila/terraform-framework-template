...
| [`terraform/resources.tf`](terraform/resources.tf) | **The dumb file.** Pure `each.value["key"]` lookups + dynamic blocks. No computation. |
...
| Single-optional sub-object (becomes splat-on-optional dynamic block in resources.tf) | `rotation`, `certificate`, `pet` in [`variables.tf`](terraform/variables.tf) |
| `<! Note: ... !>` inline comments documenting omitted/computed fields | throughout |
| Data-source-injection pattern | [`data.tf`](terraform/data.tf) → [`locals.tf`](terraform/locals.tf) → [`resources.tf`](terraform/resources.tf) |
...
| Iterative-children resources via for_each on flattened map | `local_file.manifest`, `null_resource.lifecycle_hook` in [`resources.tf`](terraform/resources.tf) |
| Filtered for_each for conditional resource creation (0..1 per env) | `time_rotating.environment_rotation`, `tls_self_signed_cert.environment` in [`resources.tf`](terraform/resources.tf) |
| Splat-on-optional dynamic block (`each.value["foo"][*]`) | `dynamic "subject"` block in [`resources.tf`](terraform/resources.tf) |
...
```sh
# Smallest possible exercise.
cp examples/single-environment/terraform.tfvars.example terraform/terraform.tfvars
make ci
```

`make ci` runs `terraform fmt -check`, `init`, `validate`, and `test` — the full local validation gate. State produced by `terraform test` lives inside the test sandbox and is torn down on completion.

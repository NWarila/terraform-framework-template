...
- **Template-tier baseline entries must resolve on disk.** `baseline-manifest.json` is load-bearing for derivative framework drift gates and covers only stable scaffold files, not framework-specific Terraform implementation details or optional release automation.

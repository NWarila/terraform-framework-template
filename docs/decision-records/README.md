...
- `org/` — byte-identical mirrors of org-baseline ADRs from [`nwarila-platform/.github`](https://github.com/nwarila-platform/.github). Apply to every repo in the org regardless of stack. Drift-gated by [`.github/workflows/drift-gate.yaml`](../../.github/workflows/drift-gate.yaml).
...
This framework-example is a do-nothing reference framework. It mirrors the org baseline so it participates in the same quality-gate fleet as every other repo in the portfolio, but doesn't author its own template-tier or repo-tier decisions.

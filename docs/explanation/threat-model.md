...
- **Public repo accidentally tracking secrets.** A future contributor commits a `terraform.tfvars` containing real credentials into the framework's runner-inventory fixture tree. Mitigation: the template's `.gitignore` denies all paths by default (per org ADR-0003); explicit allowlist entries gate every tracked file. Pre-commit hooks + `gitleaks` (in iac-security) catch staged secrets that slip past the allowlist.
...
- **State backend unavailability.** A derivative framework's remote backend outage can prevent state locking or writes. Mitigation: this template fails fast because it has no remote backend. Real frameworks must document backend availability expectations and recovery procedures alongside the backend they choose.
...
- [ADR-template/0003](../decision-records/template/0003-separate-framework-code-from-runner-inventory.md) — establishes the framework/runner ownership boundary used by overlay validation.

...
- [Template-tier ADR-template/0001 (in `terraform-runner-template`)](https://github.com/NWarila/terraform-runner-template/blob/main/docs/decision-records/0001-pin-terraform-and-provider-versions-exactly.md) — establishes exact-pinning of Terraform CLI and provider versions. Direct mitigation for "silent uptake of a compromised release" listed above.
- [Template-tier ADR-template/0002 (in `terraform-runner-template`)](https://github.com/NWarila/terraform-runner-template/blob/main/docs/decision-records/0002-mandate-s3-state-backend.md) — establishes S3 + native locking + OIDC-only auth as the mandatory state backend. Direct mitigation for state-file confidentiality, integrity, and availability concerns.
